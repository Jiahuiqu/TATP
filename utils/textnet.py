...
class Adapter(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x + residual  # 残差连接
        return x
class Text_Net_Adapter(nn.Module):
    def __init__(self, adapter_hidden_dim):
        super(Text_Net_Adapter, self).__init__()
        ...
        self.text_net.load_state_dict(torch.load("utils/text_encoder.pth"))
        self.adapter = Adapter(512, adapter_hidden_dim)
        for param in self.text_net.parameters():
            param.requires_grad = False

    def forward(self, text):
        x = self.text_net(text)
        x = self.adapter(x)

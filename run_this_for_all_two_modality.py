...
def evaluate_model(model,epoch):
    ...
    y_test = 0
    with torch.no_grad():
        for step, (lidar_out, HS_out, lable_out, ik_out) in enumerate(data_loader):
            # Move data to device
            lidar_out = lidar_out.type(torch.float).to(DEVICE)
            HS_out = HS_out.type(torch.float).to(DEVICE)
            lable_out = lable_out.type(torch.LongTensor).to(DEVICE) - 1
...
################################################################################################################################################
    ...
    from modelAll_any_2_new import modelAll
    model = modelAll(DEVICE, [30, 4],pache_size=patchsize_inthis,num_classes=7).to(DEVICE)

    modelName = 'modelAll_any_2'

    datasetoff = 3 # houston2013 muffl augsburg

    file_name = f"newresult/model_{modelName}_{datasetoff}_000.txt"

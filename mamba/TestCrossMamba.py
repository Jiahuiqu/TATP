...
try:
    from mamba.IJCAI2025.mamba2.ssd_minimal import selective_scan_chunk_fn
except:
...
class CrossMamba(nn.Module):
    def __init__(self, d_model=96, d_state=16, ssm_ratio=2.0, dt_rank="auto", dropout=0.0):
        ...
        # in proj ============================
        self.in_proj1 = nn.Linear(d_model, d_inner * 2, bias=bias)
        self.in_proj2 = nn.Linear(d_model, d_inner, bias=bias)
    ...
    def forward(self, x1, x2):
        ...
        x1 = self.in_proj1(self.in_norm(x1))  # [b, h, w, d_model] -> [b, h, w, d_inner * 2]
        x2 = self.in_proj2(self.in_norm(x2))  # [b, h, w, d_model] -> [b, h, w, d_inner]

        x1, z = x1.chunk(2, dim=-1)  # x1:[b, h, w, d_inner]   z:[b, h, w, d_inner]
        z = self.act(z)  # nn.SiLU

        x1 = x1.permute(0, 3, 1, 2).contiguous()  # x1: [b, d_inner, h, w]
        x2 = x2.permute(0, 3, 1, 2).contiguous()

        x1 = self.conv2d(x1)  # x1:[b, d_inner, h, w]
        x2 = self.conv2d(x2)

        x1 = self.act(x1)  # nn.SiLU
        x2 = self.act(x2)

        y1 = self.selectiveScan(x1)
        y2 = self.selectiveScan(x2)

        y = y1 * z + y2 * z  # y: [b, h, w, d_inner]

        out = self.dropout(self.out_proj(y)).permute(0, 3, 1, 2)  # out:[b, d_model, h, w]
        return residualTemp + out
...
def main():
    ...
    x1 = torch.randn(1, 64, 40, 40).to(device)
    x2 = torch.randn(1, 64, 40, 40).to(device)

    model = CrossMamba(d_model=64).to(device)
